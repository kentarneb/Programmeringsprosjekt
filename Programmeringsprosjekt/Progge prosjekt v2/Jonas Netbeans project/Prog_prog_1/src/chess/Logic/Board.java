...
public class Board implements Serializable {
    ...
    private ArrayList<Flytt> moveRecord = new ArrayList<Flytt>();
    ...
    private void initPieces() {
        ...
        String black_bishop_image = filepath + "BlackBishop" + suffix;
        ...
        if (p instanceof King && board[x][y] instanceof Rook && p.isWhite() == board[x][y].isWhite()) {
            if (x == 0 && y == 0) {
                if (gr.getLegalMoves(p, b)[y][x] == 1) {
                    ...
                    p.setCords(2, 0);
                    Rook.setCords(3, 0);
                    moveRecord.add(new Flytt(p.getX(), p.getY(), x, y, p, Rook, true));
                }
            } else if (x == 7 && y == 0) {
                if (gr.getLegalMoves(p, b)[y][x] == 1) {
                    Rook = board[x][y];
...

            {
                ...
                if (killedPiece == null) {
                    moveRecord.add(new Flytt(p.getX(), p.getY(), x, y, p));


                } else {
                    moveRecord.add(new Flytt(p.getX(), p.getY(), x, y, p, killedPiece));
    ...
    public Piece lastMovedPiece() {
        return moveRecord.get(moveRecord.size() - 2).getBrikke();
    }
    ...
    public boolean undoMove() {
        if (moveRecord.size() > 0) {
            Flytt move = moveRecord.get(moveRecord.size() - 1);
            resetDrawn(
                    move); //Only happens when it should.
            board[move.getFraX()][move.getFraY()] = board[move.getTilX()][move.getTilY()];
            board[move.getTilX()][move.getTilY()] = null;
            move.getBrikke().setCords(move.getFraX(), move.getFraY());

            try {
                Piece killedPiece = move.getEliminertBrikke();
                board[move.getTilX()][move.getTilY()] = killedPiece;
                killedPiece.setCords(move.getTilX(), move.getTilY());
    ...
    private void resetDrawn(Flytt move) {
        if (move.getBrikke() instanceof Pawn) {
            Pawn helper = (Pawn) move.getBrikke();
            ...
            move.setBrikke(helper);


        }
        if (move.getBrikke() instanceof King) {
            King helper = (King) move.getBrikke();
            helper.decrementNumberOfMoves();
        ...
        if (move.getBrikke() instanceof Rook) {
            Rook helper = (Rook) move.getBrikke();
            helper.decrementNumberOfMoves();
